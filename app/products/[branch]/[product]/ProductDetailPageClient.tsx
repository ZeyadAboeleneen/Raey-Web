...
interface Props {
  /** Pre-fetched product from the server component. When provided, skips the
   *  initial API fetch so the same product is not requested twice. */
...
export default function ProductDetailPageClient({ initialProduct }: Props) {
  ...
  const handleWhatsAppOrder = () => {
    ...
    const branchKey = collectionDetails[branch]?.titleKey
    const branchNameAr = branchKey ? (tAr[branchKey as keyof typeof tAr] || branch) : branch
    ...
    const priceToDisplay = isRentBranch
      ...
      : (() => {
          const referenceSize = product.sizes[selectedSize] || product.sizes[0]
    ...
    if (isCustomSizeMode) {
      ...
      message += `${tAr.waMsgSize} ${product.sizes[selectedSize]?.size || "One Size"}\n`
    }
  ...
  // If initialProduct was provided, skip cache/API fetch for the main product.
  // Only fetch reviews and related products.
  useEffect(() => {
    if (!branch || !productId) return

    if (initialProduct) {
      void fetchReviewsForProduct(initialProduct.id)
