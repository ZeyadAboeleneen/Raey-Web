...
export default function WeddingPage() {
  ...
  // Mood rotation
  useEffect(() => {
    const id = window.setInterval(() => { setMoodIndex((prev) => (prev + 1) % moodWords.length) }, 1500)
  ...
  const handleNewsletterSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok || (data && data.error)) { toast({ title: t("stayUpdated"), description: (data && data.error) || "Failed to subscribe. Please try again." }); return }
      setNewsletterEmail("")
      toast({ title: t("stayUpdated"), description: data && data.alreadySubscribed ? "You are already subscribed to our offers." : "You have been subscribed. We will email you future offers." })
    } catch (error) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (debouncedQuery.trim()) {
      ...
      const scoreProduct = (p: Product) => {
        ...
        for (const term of terms) { if (name.includes(term)) score += 2; if (description.includes(term)) score += 1 }
        if (p.isBestseller) score += 0.5
        if (p.isNew) score += 0.25
        return score
      }
      const scored = result.map(p => ({ p, s: scoreProduct(p) }))
  ...
  const closeSizeSelector = () => {
    ...
    setTimeout(() => { setSelectedProduct(null); setSelectedSize(null); resetMeasurements(); setIsCustomSizeMode(true); setMeasurementUnit("cm"); setConfirmMeasurements(false) }, 300)
  }
  ...
  const addToCart = () => {
    ...
    if (!isCustomSizeMode && selectedSize) {
      if (selectedSize.stockCount !== undefined && selectedSize.stockCount < quantity) { alert(`Insufficient stock for ${selectedProduct.name} - Size ${selectedSize.size}. Available: ${selectedSize.stockCount}, Requested: ${quantity}`); return }
      if (selectedSize.stockCount !== undefined && selectedSize.stockCount === 0) { alert(`Size ${selectedSize.size} is out of stock`); return }
    }
    let firstSize: ProductSize | null = selectedProduct.sizes?.[0] || null
    const fallbackSize: ProductSize = { size: "custom", volume: measurementUnit, discountedPrice: selectedProduct.packagePrice || (firstSize?.discountedPrice ?? 0), originalPrice: firstSize?.originalPrice ?? 0 }
    ...
    cartDispatch({
      type: "ADD_ITEM",
      payload: {
        id: `${selectedProduct.id}-${isCustomSizeMode ? "custom" : baseSize.size}`, productId: selectedProduct.id,
        name: selectedProduct.name, price: computedPrice, originalPrice: baseSize.originalPrice,
        size: isCustomSizeMode ? "custom" : baseSize.size, volume: isCustomSizeMode ? measurementUnit : baseSize.volume,
        image: selectedProduct.images[0], category: selectedProduct.category, quantity,
  ...
  const handlePageChange = (newPage: number) => {
    setPage(newPage)
    setTimeout(() => { allProductsRef.current?.scrollIntoView({ top: 0, behavior: "smooth" } as any) }, 100)
  }
  ...
  const handleFavoriteClick = (e: React.MouseEvent, product: Product) => {
    ...
    if (isFavorite(product.id)) { removeFromFavorites(product.id) } else {
      ...
      addToFavorites({
        id: product.id, name: product.name, price, image: product.images[0], category: product.category,
        rating: product.rating, isNew: product.isNew, isBestseller: product.isBestseller, sizes: product.sizes,
        isGiftPackage: product.isGiftPackage, packagePrice: product.packagePrice,
        packageOriginalPrice: product.packageOriginalPrice, giftPackageSizes: product.giftPackageSizes,
      })
    }
  }
  ...
  return (
    <div className="min-h-screen bg-white">
            ...
            <div className="p-6">
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <p className="text-gray-600 text-sm">Select your size to {isRentCategory(selectedProduct.category) ? "rent" : "buy"}</p>
                </div>
                <div className="flex">
              ...
              <div className="flex items-center mb-6">
                ...
                <div>
                  ...
                  <div className="flex items-center mt-1">
                    {[...Array(5)].map((_, i) => (<Star key={i} className={`h-4 w-4 ${i < Math.floor(selectedProduct.rating) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`} />))}
              ...
              <div className="flex justify-between items-center py-4 border-t border-gray-100">
                <div>
                  ...
                  <div className="text-xl font-medium ml-2">
                    {(() => {
                      const qty = quantity
                      if (selectedSize) {
                        const unitOriginal = selectedSize.originalPrice || 0; const unitDiscount = selectedSize.discountedPrice || 0
                        const hasDiscount = unitOriginal > 0 && selectedSize.discountedPrice !== undefined && unitDiscount < unitOriginal
                        const totalOriginal = unitOriginal * qty; const totalPrice = (hasDiscount ? unitDiscount : unitOriginal || unitDiscount) * qty
                        if (hasDiscount) return (<><span className="line-through text-gray-400 mr-2 text-lg">{formatPrice(totalOriginal)}</span><span className="text-red-600 font-bold">{formatPrice(totalPrice)}</span></>)
                        return <>{formatPrice(totalPrice)}</>
                      }
                      if (isCustomSizeMode && selectedProduct.sizes?.length > 0) { const fs = selectedProduct.sizes[0]; return <>{formatPrice((fs.discountedPrice || fs.originalPrice || 0) * qty)}</> }
                      return <>{formatPrice(getSmallestPrice(selectedProduct.sizes) * qty)}</>
                    })()}
                  </div>
                </div>
                <Button onClick={() => { if (!selectedProduct || selectedProduct.isOutOfStock) return; if (!isCustomSizeMode) { addToCart(); return }; if (!isMeasurementsValid) { alert("Please complete your custom measurements"); return }; setShowCustomSizeConfirmation(true) }} className={`flex items-center rounded-full px-6 py-5 ${selectedProduct?.isOutOfStock ? 'bg-gray-400 cursor-not-allowed opacity-60' : 'bg-black hover:bg-gray-800'}`} disabled={selectedProduct?.isOutOfStock || (isCustomSizeMode ? !isMeasurementsValid : !selectedSize)}>
                  ...
                  {selectedProduct?.isOutOfStock ? "Out of Stock" : isRentCategory(selectedProduct.category) ? "Rent Now" : "Buy Now"}
                </Button>
              </div>
            </div>
          </motion.div>
      ...
      <AlertDialog open={showCustomSizeConfirmation} onOpenChange={setShowCustomSizeConfirmation}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2"><AlertCircle className="h-5 w-5 text-amber-500" />Confirm Your Custom Size</AlertDialogTitle>
            <AlertDialogDescription className="space-y-2 pt-2">
              <p>These are the custom measurements we will use for this gown:</p>
              <div className="bg-gray-50 p-4 rounded-lg space-y-1 text-sm">
                <div className="grid grid-cols-2 gap-2">
                  <span><strong>Shoulder:</strong> {measurements.shoulder} {measurementUnit}</span><span><strong>Bust:</strong> {measurements.bust} {measurementUnit}</span>
                  <span><strong>Waist:</strong> {measurements.waist} {measurementUnit}</span><span><strong>Hips:</strong> {measurements.hips} {measurementUnit}</span>
                  <span><strong>Sleeve:</strong> {measurements.sleeve} {measurementUnit}</span><span><strong>Length:</strong> {measurements.length} {measurementUnit}</span>
                </div>
              </div>
              <p className="text-amber-600 font-medium">If anything looks incorrect, choose &quot;Review Again&quot; to adjust.</p>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setShowCustomSizeConfirmation(false)}>Review Again</AlertDialogCancel>
            <AlertDialogAction onClick={() => { addToCart(); setShowCustomSizeConfirmation(false) }} className="bg-black hover:bg-gray-800">Confirm & Send on WhatsApp</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} className="relative h-[60vh] md:h-[70vh] flex items-center justify-center overflow-hidden">
        ...
        <motion.div className="relative z-10 max-w-3xl mx-auto px-4 text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <h1 className="text-5xl sm:text-6xl md:text-8xl font-extrabold tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-white via-rose-200 to-white mb-8 leading-[1.2] pb-4">Wedding Dresses</h1>
          <p className="text-sm sm:text-base text-gray-200 max-w-2xl mx-auto">Discover your dream wedding gown from our curated collections.</p>
        </motion.div>
      </motion.section>
      ...
      {/* ─── All Products (Search + Filters + Grid + Pagination) ─── */}
        ...
        <div className="container mx-auto px-4 sm:px-6">
          ...
          <div className="mb-8 space-y-4">
            <div className="flex flex-col sm:flex-row gap-3 max-w-4xl mx-auto">
              ...
              <div className="relative sm:w-56">
                <select value={selectedCollection} onChange={(e) => setSelectedCollection(e.target.value)} className="w-full appearance-none rounded-full border border-gray-200 bg-white/90 py-3 pl-5 pr-10 text-sm tracking-wide focus:outline-none focus:border-black transition-colors cursor-pointer">
                  <option value="">All Collections</option>
                  {COLLECTIONS_FILTER.map(c => (<option key={c.slug} value={c.slug}>{c.label}</option>))}
                </select>
            ...
            <div className="text-center text-sm text-gray-500">
              {debouncedQuery || selectedCollection || selectedPriceRanges.length > 0 ? `Showing ${filteredProducts.length} of ${allProducts.length} products` : `Showing all ${allProducts.length} products`}
            </div>
          </div>
          ...
          {cacheLoading ? (<div className="flex justify-center py-16 text-gray-500 text-sm">Loading products...</div>) : filteredProducts.length === 0 ? (
            <div className="text-center py-16">
              ...
              <Button onClick={() => { setSearchQuery(""); setSelectedCollection(""); setSelectedPriceRanges([]) }} className="mt-4 bg-black text-white hover:bg-gray-800 rounded-full">Clear Filters</Button>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4 md:gap-6">
                {paginatedProducts.map((product, index) => renderProductCard(product as Product, index))}
              </div>
                  ...
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-1 max-w-[210px] sm:max-w-none">
                      {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                        <Button key={p} variant={page === p ? "default" : "ghost"} size="sm" onClick={() => handlePageChange(p)} className={`w-9 h-9 rounded-full p-0 transition-all duration-200 ${page === p ? "bg-black text-white shadow-md scale-110" : "hover:bg-rose-50 hover:text-rose-600 text-gray-500"}`}>{p}</Button>
                      ))}
                    </div>
        ...
        <div className="container mx-auto px-6">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }} viewport={{ once: true }} className="order-1 md:order-2">
              ...
              <p className="text-gray-600 mb-6 leading-relaxed">Discover a carefully curated selection of wedding dresses designed to make your special day unforgettable.</p>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }} viewport={{ once: true }} className="order-2 md:order-1">
              <div className="w-full h-64 md:h-96 relative"><Image src="/wedding.jpg" alt="Raey Wedding" fill className="object-cover rounded-lg" priority /></div>
            </motion.div>
          </div>
        </div>
      </motion.section>
