...
import { Star, ShoppingCart, X, Heart, AlertCircle, Search, ChevronDown, Package, ArrowRight, Sparkles, ChevronLeft, ChevronRight, Plus, MessageCircle } from "lucide-react"
    ...
    <div className="min-h-screen bg-white">
      ...
      <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} className="relative h-[60vh] md:h-[70vh] flex items-center justify-center overflow-hidden">
        <motion.div className="absolute inset-0 z-0" animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 15, ease: "easeInOut", repeat: Infinity }}>
          <Image src={heroImages.soiree} alt="Soiree background" fill priority sizes="100vw" className="object-cover" />
        ...
        <motion.div className="relative z-10 max-w-3xl mx-auto px-4 text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <h1 className="text-5xl sm:text-6xl md:text-8xl font-extrabold tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-white via-rose-200 to-white mb-8 leading-[1.2] pb-4">{t("soireeCollectionsTitle")}</h1>
          <p className="text-sm sm:text-base text-gray-200 max-w-2xl mx-auto">{t("soireeCollectionsDesc")}</p>
        </motion.div>
      </motion.section>
            ...
            <div className="relative group/carousel">
              ...
              <div className="overflow-hidden cursor-grab active:cursor-grabbing" ref={emblaRefNew}>
                <div className="flex -ml-4">
                  {displayedNewProducts.map((product, index) => (<div key={product._id} className="flex-[0_0_45%] sm:flex-[0_0_30%] md:flex-[0_0_25%] lg:flex-[0_0_20%] xl:flex-[0_0_18%] min-w-0 pl-4 h-full">{renderProductCard(product as Product, index)}</div>))}
                  {newProducts.length > visibleNewCount && (
                    <div className="flex-[0_0_45%] sm:flex-[0_0_30%] md:flex-[0_0_25%] lg:flex-[0_0_20%] xl:flex-[0_0_18%] min-w-0 pl-4 h-full self-stretch">
                      <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setVisibleNewCount(prev => prev + 10)} className="w-full h-full min-h-[300px] flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-2xl bg-white/50 hover:bg-white hover:border-black transition-all group"><div className="h-12 w-12 rounded-full bg-gray-100 flex items-center justify-center mb-4 group-hover:bg-black group-hover:text-white transition-colors"><Plus className="h-6 w-6" /></div><span className="text-sm font-medium tracking-wide uppercase">{t("viewAll")}</span><span className="text-xs text-gray-500 mt-1">{newProducts.length - visibleNewCount} more</span></motion.button>
                    </div>
                  )}
                </div>
              </div>
      ...
      <motion.section ref={bestSellersRef} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.6 }} viewport={{ once: true, amount: 0.3 }} className="py-16 bg-white overflow-hidden">
        <div className="container mx-auto px-6">
          ...
          {bestSellersRent.length === 0 && !cacheLoading ? (<div className="flex justify-center py-10 text-gray-500 text-sm">{t("noBestRentals")}</div>) : (
            <div className="relative group/carousel">
              <div className="absolute top-1/2 -left-4 sm:-left-6 -translate-y-1/2 z-10">
