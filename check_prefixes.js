...
(async () => {
  ...
  const sampleMissing = await pool.request().query("SELECT TOP 10 ID, Item_name FROM Items WHERE Category_id IN (1,6) AND ID NOT IN (SELECT DISTINCT ModelTypeID FROM Booking WHERE BranchID IS NOT NULL) AND ID NOT IN (SELECT ItemID FROM ItemStores)");
